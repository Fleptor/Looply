...
function getBrowserStorage(type) {
    try {
        ...
        const testKey =
            `__looply_${type}_storage_test__`;
        ...
        throw new Error(
            `${type}Storage is not available in this browser.`
        );
    }
}
...
function readValue(
    ...
    try {
        ...
        console.error(
            `Unable to parse storage value for "${storageKey}".`,
            error
        );
...
function writeValue(storage, key, value) {
    ...
    if (value === undefined) {
        throw new TypeError(
            "Cannot store an undefined value."
        );
    }
    ...
    try {
        ...
        throw new Error(
            `Unable to convert "${storageKey}" value to JSON.`
        );
    }

    if (serializedValue === undefined) {
        throw new TypeError(
            `The value for "${storageKey}" cannot be stored.`
        );
    }
    ...
    try {
        ...
        throw new Error(
            `Unable to save storage value for "${storageKey}".`
        );
    }
