...
export function getUserById(
    ...
    return getAllUsers().find((user) => {
        ...
        const canReturnDeleted =
            includeDeleted || user.isDeleted !== true;
...
function assertUniqueStudentFields(
    users,
    studentData,
    ignoredUserId = null
) {
    ...
    const nationalIdExists = users.some((user) => (
        user.id !== ignoredUserId &&
        nationalId !== "" &&
...
export function updateUser(userId, changes = {}) {
    ...
    const nextUsername = changes.username === undefined
        ? currentUser.username
        : normalizeText(changes.username).toLowerCase();

    const nextNationalId = changes.nationalId === undefined
